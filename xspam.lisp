...
(defun all-upper-case? (s)
  (loop for c across s always (or (not (alpha-char-p c)) (upper-case-p c))))
  ...
  (let
      ((pname
        (cond
          ...
          ((symbolp spec)
            (if (all-upper-case? (symbol-name spec))
              (error "symbol name ~s cannot be all upper case" (symbol-name spec))
